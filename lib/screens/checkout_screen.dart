...
class CheckoutScreen extends BaseRoute {
  ...
  CheckoutScreen({a, o, this.cartController}) : super(a: a, o: o, r: 'CheckoutScreen');

  @override
  _CheckoutScreenState createState() => _CheckoutScreenState(cartController: cartController);
}
...
class _CheckoutScreenState extends BaseRouteState {
          ...
          body: Padding(
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  Text(
                    global.userProfileController.addressList != null && global.userProfileController.addressList.length > 0 ? "${AppLocalizations.of(context).lbl_shipping_to} Shipping to " : "${AppLocalizations.of(context).txt_no_address}",
                    style: subHeadingStyle,
                  ),
                  Container(
                    height: MediaQuery.of(context).size.height * 0.3,
                    width: MediaQuery.of(context).size.width,
                    child: global.userProfileController.addressList != null && global.userProfileController.addressList.length > 0
                        ? ListView.builder(
                            shrinkWrap: true,
                                  ...
                                  child: AddressInfoCard(
                                    ...
                                    onChanged: (value) {
                                      setState(() {
                                        ...
                                        _selectAddressForCheckout(selectedAddressId: _selectedAddress.addressId);
                                      });
                                    },
                                  ),
                                ),
                  ...
                  _isClosingTime == false && global.nearStoreModel.storeOpeningTime != null && global.nearStoreModel.storeOpeningTime != '' && global.nearStoreModel.storeClosingTime != null && global.nearStoreModel.storeClosingTime != '' && DateTime.now().isAfter(_openingTime) && DateTime.now().isBefore(_closingTime.subtract(Duration(hours: 1)))
                      ? SizedBox(
                          ...
                          child: SwitchListTile(
                            ...
                            value: _membershipStatus.status == 'running' ? true : false,
                            ...
                            title: Text(
                              _membershipStatus.status == 'running' ? '${AppLocalizations.of(context).btn_instant_delivery}' : "${AppLocalizations.of(context).btn_req_instant_delivery}",
                      ...
                      : SizedBox(
                          ...
                          child: Card(
                              ...
                              child: Padding(
                                ...
                                child: Text('Instant delivery not available as store closing time is near.'),
                              ))),
                                      ...
                                      SizedBox(
                                        ...
                                        child: Center(
                                          child: DropdownButton(
                                            ...
                                            items: _timeSlotList.map<DropdownMenuItem<TimeSlot>>((TimeSlot timeSlot) {
                                              return DropdownMenuItem<TimeSlot>(
                                                value: timeSlot,
                                                enabled: timeSlot.availibility == "available" ? true : false,
                                                child: Text(
                                                  timeSlot.timeslot,
                                                  style: timeSlot.availibility == "available"
                ...
                children: [
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Text(
                        cartController.cartItemsList != null ? "${global.appInfo.currencySign} ${cartController.cartItemsList.totalPrice}" : '${global.appInfo.currencySign} 0',
                        style: TextStyle(
                          fontWeight: FontWeight.bold,
                        ),
                      )
                  ...
                  ConfirmationSlider(
                      ...
                      text: "${AppLocalizations.of(context).txt_swipe_to_order}",
  ...
  _checkMembershipStatus() async {
    try {
      ...
      print("Exception - checkout_screen.dart - _checkMembershipStatus():" + e.toString());
    }
  }
  ...
  _getTimeSlotList() async {
    try {
      ...
      print("Exception - checkout_screen.dart - _getTimeSlotList():" + e.toString());
    }
  }

  _makeOrder() async {
    try {
      if (_selectedAddress == null || (_selectedAddress != null && _selectedAddress.addressId == null)) {
      ...
      } else if (_selectedDate == null && _membershipStatus.status != 'running') {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          backgroundColor: Theme.of(context).primaryColor,
          content: Text(
      ...
      } else if (_selectedTimeSlot == null && _membershipStatus.status != 'running') {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          backgroundColor: Theme.of(context).primaryColor,
          content: Text(
            '${AppLocalizations.of(context).txt_select_time_slot}',
            textAlign: TextAlign.center,
          ),
          duration: Duration(seconds: 2),
        ));
        ...
        bool isConnected = await br.checkConnectivity();
        if (isConnected) {
          await apiHelper.makeOrder(selectedDate: _selectedDate, selectedTime: _membershipStatus.status == 'running' ? 'instant' : _selectedTimeSlot.timeslot).then((result) async {
            if (result != null) {
              if (result.status == "1") {
  ...
  _selectAddressForCheckout({int selectedAddressId}) async {
    try {
      showOnlyLoaderDialog();
      bool isConnected = await br.checkConnectivity();
      if (isConnected) {
        await apiHelper.selectAddressForCheckout(selectedAddressId).then((result) async {
          if (result != null) {
            if (result.status == "1") {
              ScaffoldMessenger.of(context).showSnackBar(SnackBar(
                backgroundColor: Theme.of(context).primaryColor,
      ...
      print("Exception - checkout_screen.dart - _selectAddressForCheckout():" + e.toString());
    }
  }
}
