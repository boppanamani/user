...
class UserInfoTile extends StatefulWidget {
  ...
  UserInfoTile({@required this.heading, this.value, this.leadingIcon, this.onPressed, this.key}) : super();

  @override
  _UserInfoTileState createState() => _UserInfoTileState(heading: heading, value: value, leadingIcon: leadingIcon, onPressed: onPressed, key: key);
}
...
class UserOrdersDashboardBox extends StatefulWidget {
  ...
  @override
  _UserOrdersDashboardBoxState createState() => _UserOrdersDashboardBoxState(heading: heading, value: value);
}
...
class _UserInfoTileState extends State<UserInfoTile> {
  ...
  _UserInfoTileState({@required this.heading, this.value, this.leadingIcon, this.onPressed, this.key});
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(
                    heading,
                    style: textTheme.bodyText1.copyWith(fontWeight: value == null ? FontWeight.bold : FontWeight.normal, fontSize: 15),
                  ),
                ],
              ),
              SizedBox(height: 8),
              value == null
                  ...
                  : Text(
                      value,
                      style: textTheme.bodyText1.copyWith(fontWeight: FontWeight.bold, fontSize: 15),
                    ),
...
class _UserProfileScreenState extends BaseRouteState {
        ...
        body: _isDataLoaded
            ? GetBuilder<UserProfileController>(
                ...
                builder: (value) => RefreshIndicator(
                      onRefresh: () async {
                        _isDataLoaded = false;
                        global.userProfileController.currentUser = new CurrentUser();
                        setState(() {});
                        await _getMyProfile();
                      },
                      child: Padding(
                        ...
                        child: SingleChildScrollView(
                          child: Column(
                            ...
                            children: [
                              ...
                              Center(
                                child: Text(
                                  global.userProfileController.currentUser.name != null && global.userProfileController.currentUser.name != '' ? global.userProfileController.currentUser.name : 'User',
                                  style: textTheme.headline6,
                                ),
                              ),
                              // Padding(
                              ...
                              //   ),
                              // ),
                              UserInfoTile(heading: "${AppLocalizations.of(context).lbl_phone_number}", value: global.userProfileController.currentUser.userPhone),
                              SizedBox(height: 8),
                              UserInfoTile(
                                key: UniqueKey(),
                                heading: "${AppLocalizations.of(context).txt_address}",
                                onPressed: () {
                                ...
                                value: global.userProfileController.addressList != null && global.userProfileController.addressList.length > 0 ? global.userProfileController.addressList[0].houseNo : '${AppLocalizations.of(context).txt_nothing_to_show}',
                              ),
                              SizedBox(height: 8),
                              UserInfoTile(
                                heading: "${AppLocalizations.of(context).lbl_email}",
                                value: global.userProfileController.currentUser.email,
                              ),
                              SizedBox(height: 16),
                              UserInfoTile(
                                heading: "${AppLocalizations.of(context).lbl_reset_password}",
                                onPressed: () {
  ...
  _getMyProfile() async {
    try {
      ...
      print("Exception - UserProfileScreen.dart - _getMyProfile():" + e.toString());
    }
  }
              ...
              children: [
                ...
                Center(
                  child: Padding(
                    ...
                    child: SizedBox(height: 50, width: MediaQuery.of(context).size.width - 60, child: Card()),
                  ),
                ),
                Padding(
                  padding: const EdgeInsets.symmetric(vertical: 32.0),
                  child: Row(
                    ...
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      SizedBox(height: 80, width: (MediaQuery.of(context).size.width - 30) / 3, child: Card()),
                      SizedBox(height: 80, width: (MediaQuery.of(context).size.width - 30) / 3, child: Card()),
                      SizedBox(height: 80, width: (MediaQuery.of(context).size.width - 30) / 3, child: Card()),
                    ],
                  ),
                ...
                Padding(
                  ...
                  child: SizedBox(height: 70, width: MediaQuery.of(context).size.width, child: Card()),
                ),
                Padding(
                  padding: const EdgeInsets.only(top: 10, left: 8, right: 8),
                  child: SizedBox(height: 70, width: MediaQuery.of(context).size.width, child: Card()),
